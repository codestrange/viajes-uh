...
<script>
    export default {
        data () {
            return {
                items: [
                    {id:0, username: 'Fallo', email:'Fallo'},
                ],
                fields: [
        ...
        methods: {
            ...
            loadUsers() {
                return this.$store.state.user.getUsersData().then(users => { this.items = users });
            }
        },
        mounted() {
            this.$store.state.loader.showLoading();
            this.loadUsers().then( () => { this.$store.state.loader.stopShowing() });
        }
    }
</script>
